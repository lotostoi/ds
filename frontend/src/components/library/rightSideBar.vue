...
<div class="lib-right">
    ...
    <div v-if=" oneImage && oneImage.link" class="lib-right-body">
        <transition appear-class="enter" leave-active-class="leave" enter-active-class="enter" mode="out-in">
            <div class="lib-right-cont" :style="`height:${width/oneImage.k}px; width:${width-10}px`" :key="oneImage._id">
                <img :id="oneImage.link" :data-id="oneImage._id" class="lib-right-oneImage" v-lazy="oneImage.link" :alt="oneImage.name" :name="oneImage.link" />
            </div>
        </transition>
        <div class="lib-right-name-cont" :style="`width:${width - 20}px`" v-if="oneImage">
            <div class="fields" v-if="true">
                <span>File:</span>
                <span>{{oneImage.realName}}</span>
            </div>
            <div class="fields" v-if="false">
                <input type="text" name="name" id="right-name" placeholder="Input new name..." required />
                <button>Save</button>
            </div>
            <div class="fields">
                <span>Titel:</span>
